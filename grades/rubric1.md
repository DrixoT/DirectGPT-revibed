...
Partial Success: a single click on a word in the text does NOT select it (mouse down+up on "pictures"/"reading" leaves the badge absent and adds no highlight). Selection of a text word only happens on double-click or on a drag across the word. Once selected, the light-blue highlight, the badge "Apply to 1 selected element" and its "✕" clear control all work correctly (Escape also clears). Fix: treat a plain click inside the text view as a word selection, as the paper describes.
...
Partial Success: accumulation itself works — holding Ctrl while drag-selecting a second word gives "Apply to 2 selected elements", and repeating the Ctrl+drag on that word toggles it back to 1. But Ctrl+*click* (and Ctrl+double-click) on a word does nothing at all: the count stays at 1 and the second word is never added, because word selection is driven only by a non-collapsed native text selection. Multi-select therefore requires the Ctrl+drag workaround.
...
Partial Success: pressing the mouse on the word `hot` and dragging (the step as written) does not start an object drag — it just performs a text selection and nothing is inserted in the prompt. The word must first be double-clicked (selected) and only then can it be dragged; with that extra step the drop works, inserts a grey chip (background rgb(227,229,233), rounded) that is visually distinct, drops correctly between words, and typing continues normally around it.
...
Partial Success: hovering a chip highlights the source word in the content (class m-hover) and Backspace after a chip deletes the whole chip in one keystroke, but copy/paste of a chip does not work — selecting the chip and pressing Ctrl+C leaves the clipboard empty and a following Ctrl+V inserts nothing, so the chip cannot be duplicated as a reference.
...
Partial Success: dragging the `console` token requires first double-clicking it to select it — pressing the mouse on an unselected token and dragging does nothing (no drag ghost, nothing inserted). After that extra step the grey chip renders correctly, the interface sends the right delimited reference (`0]console0].log(line)` plus "Keep rest of the text identical"), and the edit is confined to the referenced token; no comment line was inserted above `console.log(line);` (model output), the rest of the code was untouched.
...
Partial Success: dropping on the word `that` correctly replaces it with a grey chip containing an SVG thumbnail of the petal, and a thumbnail drag ghost follows the cursor, but there is NO hover feedback on the target word — while the shape hovers over `that` the word gets no yellow (or any) highlight; the prompt editor's markup is unchanged during the drag, so nothing signals that dropping will replace the word.
...
Partial Success: the mode itself is well indicated (the toolbar button gets an `active` state and the prompt area shows "Tool “synonym” active — select objects to apply it (Esc to leave)"), the mode persists across several applications and Escape exits it. But a single click on a word in the text does NOT trigger the tool — clicking `reading` and then `bank` in tool mode left the text unchanged; the word has to be double-clicked (or drag-selected) for the tool to fire, because text word selection only reacts to a non-collapsed native selection.
...
Failure: there is no task panel. The left column holds only the "Toolbar" of reusable prompts; there is no panel on the left of the interface for a task, no target image, and no notion of a current task or session. The only study affordance is a one-line grey strip under the content area ("Study tasks for this sample: Colour three petals with a gradient · Add a stem and two leaves · …") that lists all four tasks for the loaded sample at once; it is plain page text outside the manipulable content (so it cannot be selected as an object or dragged into the prompt), but it is not the described panel and there is no target image to attempt dragging. Section 8 is marked optional in this rubric; the study harness is essentially not implemented.
...
Partial Success: two of the four elements are present — the header "Load study sample ▾" menu pre-loads each task's starting content directly into the fixed content area (Alice, Frankenstein, two JavaScript samples, Flower, Smiley), and choosing another sample resets the content to that sample's starting version. There is no 3-minute limit (nothing times a task out) and no post-task 5-point "How close are you to the target" rating.
...
Partial Success: the whole narrated sequence completes — localized edit on `a White Rabbit` with the span pulsing, two-word `synonym` with pulsing + bold highlight of "illustrations"/"sprinted", the "synonym" tool appearing in the toolbar and applying on click, and one Undo reverting only the last tool application (`reading`). The workaround needed is selection: a plain click on a word never selects (or fires the tool), so every "click the word" step in the use case had to be done with a double-click, and the Ctrl+click for the second word had to be a Ctrl+drag across it.
...
Partial Success: steps 1, 2 and 4 behave as described — `rename to output` on the token `line` changed only that token (a <blank> prompt), the resulting "rename to output" tool re-applied to code tokens repeatedly, and Undo restored the previous state in one step. Step 3 fell short: the two `for` tokens were dragged in as two chips and the interface sent them correctly as `0]for0]` / `1]for1]` with "Keep rest of the text identical", but the returned code was byte-identical to the input, so no constrained two-block rewrite was actually produced (this looks like the model, not the interface). Also, both drags required double-clicking each token first, since unselected tokens cannot be dragged.
...
Partial Success: dragging petal B onto the existing [petal A] chip does not replace it — a second chip is inserted immediately after the first, so the prompt ends up "copy [A] [B] and place it there" with two element references. Hovering then highlights only petal A (the first chip). Drops onto plain typed words do replace the word, so only chip-on-chip replacement is missing.
...
Partial Success: when the model returns prose instead of SVG for a localized edit on one petal, the interface does not validate the output — it discards the whole image, switches the content type to text and shows the prose as the new content (the canvas is gone, and the localization to element "c0" is ignored even though the request carried it). No error or warning is shown. Recovery works: one Undo restores the exact previous SVG.
